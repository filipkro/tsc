...
class Classifier_INCEPTION:
    ...
    def _inception_module(self, input_tensor, masked, stride=1, activation='linear'):
        ...
        for i in range(len(kernel_size_s)):
            conv = keras.layers.Conv1D(filters=self.nb_filters,
                                              kernel_size=kernel_size_s[i],
                                              strides=stride, padding='same',
                                              activation=activation,
                                              use_bias=True)(input_inception)
            conv = keras.layers.Lambda((lambda x: x))(conv, mask=masked)
            conv_l.append(keras.layers.Dropout(self.dropout)(conv))
        ...
        conv_6 = keras.layers.Lambda((lambda x: x))(conv_6, mask=masked)

        conv_l.append(keras.layers.Dropout(self.dropout)(conv_6))
    ...
    def _shortcut_layer(self, input_tensor, out_tensor, masked):
        shortcut_y = keras.layers.Conv1D(filters=int(out_tensor.shape[-1]),
                                         kernel_size=1, padding='same',
                                         use_bias=False)(input_tensor)
        shortcut_y = keras.layers.Lambda((lambda x: x))(shortcut_y, mask=masked)
        shortcut_y = keras.layers.Dropout(self.dropout)(shortcut_y)
    ...
    def build_model(self, input_shape, nb_classes):
        ...
        for d in range(self.depth):
            ...
            if self.use_residual and d % 3 == 2:
                input_res = keras.layers.Lambda((lambda x: x))(input_res,
                                                               mask=mask)
                x = self._shortcut_layer(input_res, x, mask)
                input_res = x
        ...
        output_layer = keras.layers.Dropout(self.dropout)(output_layer)
