...
class Classifier_NET1d:
    ...
    def build_model(self):
        ...
        # conv_1d =  tf.keras.backend.expand_dims(masked, axis=-1)
        conv_1d = masked
        c1 = keras.layers.Conv1D(self.filters,self.window,padding='same')(tf.keras.backend.expand_dims(conv_1d[...,0], axis=-1))
        c2 = keras.layers.Conv1D(self.filters,self.window,padding='same')(tf.keras.backend.expand_dims(conv_1d[...,1], axis=-1))
        c1 = keras.layers.Conv1D(self.filters,self.window,padding='same')(c1)
        c2 = keras.layers.Conv1D(self.filters,self.window,padding='same')(c2)
        # c1 = tf.keras.backend.expand_dims(c1, axis=-2)
        # c2 = tf.keras.backend.expand_dims(c2, axis=-2)
        feats = keras.layers.Concatenate(axis=-1,
                                         name='concat')([c1, c2])
        print(c1)
        print(c2)
        print(feats)
        ...
        #
        #     conv_1d = keras.layers.Conv2D(f, (w, 1), padding='same',
