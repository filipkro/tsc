...
def main(args):
    ...
    for i, (inp, label) in enumerate(zip(x, y)):
        ...
        # fourier = fft(inp[:max_idx, 0], 256)
        # transforms[i, ...] = fourier[:25]
        plt.figure(int(label[0]))
        plt.plot(inp[:max_idx, 0])
        if np.max(np.abs(inp[:max_idx, 0] + 1.7)) > 0.2:
            print(f'{i} min: {np.min(inp[:max_idx, 0])} max: {np.max(inp[:max_idx, 0])}')

        # print(np.mean(x[:max_idx, -1]))
        # print()
        # mean_diffs[i, int(label[0])] = np.mean(inp[min:max, 0])
        # max_diffs[i, int(label[0])] = np.mean(inp[:max_idx, 0])
        #
        # fourier_var[i, int(label[0])] = np.var(inp[:max_idx, 0])

        inputs[i, :, int(label[0])] = np.mean(inp[:max_idx, :], axis=0)
        # fourier_var[i, int(label[0])] = np.var(fourier[:25])
        # mean_diffs[i, int(label[0])] = np.mean(inp[:max_idx, -1])
        # max_diffs[i, int(label[0])] = np.max(inp[:max_idx, -1])
